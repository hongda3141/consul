...
This token will be used in the server config for the replication token and
can also optionally be used as the agent token since it has enough permissions.
You can also create your own agent policy and token.
...
1. The IP addresses of the mesh gateways running in your VM datacenter.
