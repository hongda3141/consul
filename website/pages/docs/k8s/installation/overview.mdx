...
down to the [architecture](/docs/k8s/run.html#architecture) section.
...
`helm upgrade`. See the [Upgrading Consul on Kubernetes](/docs/k8s/run#upgrading-consul-on-kubernetes)
...
each Kubernetes node. This is explained in the [architecture](/docs/k8s/run#client-agents)
...
[accessing the Consul HTTP API](/docs/k8s/run#accessing-the-consul-http-api)
for an example.
